  ...
  return (
    <section id="projects" className="flex justify-center px-10 py-11">
      <div className="relative">
            ...
            {filteredProjects.length > 0 ? (
              filteredProjects.map((proj: any) => (
                ...
                >
                      ...
                      {proj.stack.map((sk: any) => (
                        <Label key={sk.title}>
                          <h4 className="quicksand-400 text-black relative z-30 flex gap-2 flex-wrap items-center justify-center">
                            ...
                            {(stackIcons as any)[sk.title] != null ? (stackIcons as any)[sk.title] : <img src={sk.icon} width={18} />}
                          </h4>
                        </Label>
                      ))}
                    </div>
                  ...
                  <div className="b-left absolute bottom-0 left-0 flex flex-col gap-4 w-full pr-4 justify-start items-start md:items-center lg:items-start">
                    <div className="3d-glassy-btn group/btn relative self-end mr-5 sm:block md:hidden lg:block cursor-pointer" onClick={() => toast("Coming Soon ...")}>
                    ...
                    <div className="more-info px-5 py-2 min-w-50 rounded-bl-lg rounded-tr-4xl md:rounded-bl-none lg:rounded-bl-lg bg-white/30 inset-ring-2 inset-ring-white/20 shadow-[inset_2px_2px_3px_rgba(255,255,255,0.4),inset_-2px_-2px_3px_rgba(255,255,255,0.25)]">
                      <span>{proj.status != null ? `${proj.status.split("-")[0]} • Completed` : "In Progress"}</span>
                    </div>
                  </div>
                </motion.div>
              ))
              ...
              >
                ...
                <p className="text-white/60 text-lg max-w-md mx-auto leading-relaxed">
                  I'm currently crafting something amazing for the <span className="text-white font-semibold uppercase tracking-wider">{currentFilter}</span> category. Stay tuned for updates!
                </p>
        ...
        <div className="categories absolute bg-white/20 backdrop-blur-lg rounded-4xl px-6 py-3 top-0 -translate-x-1/2 left-1/2 -translate-y-1/2 text-white font-semibold gradient-outline before:rounded-4xl shadow-[0_16px_20px_rgba(0,0,0,0.2)] max-w-[90vw] z-50">
          <div className="flex gap-2" style={{ overflowY: "auto" }}>
            <button
              className={`${currentFilter == "web" && "bg-white/20"
            ...
            <button
              className={`${currentFilter == "mobile" && "bg-white/20"
                } px-6 py-2 rounded-full cursor-pointer`}
            ...
            <button
              className={`${currentFilter == "scripts" && "bg-white/20"
                } px-6 py-2 rounded-full cursor-pointer`}
            ...
            <button
              className={`${currentFilter == "cli" && "bg-white/20"
                } px-6 py-2 rounded-full cursor-pointer`}
            ...
            <button
              className={`${currentFilter == "Software/GUI" && "bg-white/20"
                } px-6 py-2 rounded-full cursor-pointer`}
